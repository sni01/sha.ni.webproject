...
module.exports = function(middleware_params){
   ...
   app.post('/user', function(req, res){
	...
	try{
		...
		var response_object = { 
			create_new_user_status : "success",	
		...
		var user_session_object = {
			user_logged_in : true,
			user_name : user_object.name,
			user_gender : user_object.gender
		};
		req.session.user = user_session_object;		
		responseWithJsonObject(res, response_object, '/login-success', req.xhr);
	}
	catch(ex){
		console.log(ex);
		var response_object = {
			create_new_user_status : "failed",
			error : ex
		};
   ...
   app.get('/logout', function(req, res){
	...
	if (!user_session_object || !user_session_object.user_logged_in) response_object.logged_out_status = 0; // already logged out
	else {
		// user_session_object && user_session_object.user_logged_in
		user_session_object.user_logged_in = false;
		response_object.logged_out_status = 1; // change from logged in to logged out
	}
